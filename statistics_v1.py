    ...
    def stat1(self,image,boxes_ball,boxes_team,team_numbers,fps,frame_id):
     #--------statistica 1----------
        ...
        if(len(boxes_ball) >0) or (len(self.last_valid_ball) >0):
            ...
            #arbitri
            image = cv2.putText(image, "   Arbitri: "+str(int(self.possesso_palla[0]/fps))+" / " +str(int(frame_id/fps))+" s", (180, 120+(80 * 1)), cv2.FONT_HERSHEY_COMPLEX, 1, (200,200,200), 2)
            #Team 1                
            image = cv2.putText(image, "   Team 1: "+str(int(self.possesso_palla[1]/fps))+" / " +str(int(frame_id/fps))+" s", (180, 120 + (80 * 2)), cv2.FONT_HERSHEY_COMPLEX, 1, (200,200,200), 2)                        
            #Team 2               
            image = cv2.putText(image, "   Team 2: "+str(int(self.possesso_palla[2]/fps))+" / " +str(int(frame_id/fps))+" s", (180, 120 + (80 * 3)), cv2.FONT_HERSHEY_COMPLEX, 1, (200,200,200), 2)    
